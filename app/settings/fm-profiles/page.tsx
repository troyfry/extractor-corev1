...
export default function FmProfilesPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (status !== "authenticated") {
      setError("You need to sign in to save FM profiles.");
      return;
    }
    ...
    try {
      ...
      alert("FM profiles saved successfully!");
    } catch (err) {
  ...
  if (status !== "authenticated") {
    return (
      <AppShell>
        ...
        <div className="min-h-screen bg-slate-900 text-slate-50 p-8">
          <div className="max-w-4xl mx-auto">
            <p className="mb-4 text-slate-200">You need to sign in to manage FM profiles.</p>
            <button
              onClick={() => signIn(undefined, { callbackUrl: ROUTES.settings + "/fm-profiles" })}
        ...
        <div className="max-w-4xl mx-auto">
          <div className="mb-6">
            <button
              ...
              className="text-sky-400 hover:text-sky-300 text-sm mb-4 flex items-center gap-2"
            >
            ...
            <h1 className="text-3xl font-semibold mb-4">Facility Management Profiles</h1>
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    {profile.fmKey.trim() && (
                      <Link
                        href={`${ROUTES.onboardingTemplates}?fmKey=${encodeURIComponent(profile.fmKey)}&from=settings`}
                        className="text-sm text-blue-400 hover:text-blue-300 transition-colors flex items-center gap-1"
                      >
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7" />
                        </svg>
                        {profile.hasCoordinates ? "Edit Coordinates" : "Set Coordinates"}
                      </Link>
                    )}
